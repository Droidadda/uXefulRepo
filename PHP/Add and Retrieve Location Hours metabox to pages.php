...
function schc_add_post_meta_boxes(){
    add_meta_box('schc-location-hours', 'Location Hours', 'schc_location_hours_meta_box', 'location', 'side', 'default'); //Note: 'location' is a custom post type. Change this to 'page' or whatever to change where it appears.
}
...
//Checks the 4 primary locations in Syracuse. Returns true if ALL are CLOSED.
function are_all_primary_syracuse_locations_closed(){
    if ( is_location_closed(123) && is_location_closed(126) && is_location_closed(124) && is_location_closed(127) ){
        return true;
    }
...
add_filter('body_class', 'schc_body_classes');
function schc_body_classes($classes){
    if ( get_post_type() == 'location' ){
        $classes[] = ( is_location_open() )? 'location-open' : 'location-closed';
    }
